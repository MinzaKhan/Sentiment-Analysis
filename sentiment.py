...
stopwords = set(w.rstrip() for w in open('dataset/stopwords.txt'))

positive_reviews = BeautifulSoup(open('dataset/positive.review.txt').read())
positive_reviews = positive_reviews.findAll('review_text')

negative_reviews = BeautifulSoup(open('dataset/negative.review.txt').read())
...
def update_one(comment):
    f=open('dataset/comments.txt','a')
    f.write('\n'+'<review><review_text>'+comment+'</review_text></review>')
    f.close()

    comments_user = BeautifulSoup(open('dataset/comments.txt').read())
...
def update_two(comment):
    f=open('dataset/commentstwo.txt','a')
    f.write('\n'+'<review><review_text>'+comment+'</review_text></review>')
    f.close()

    comments_usertwo = BeautifulSoup(open('dataset/commentstwo.txt').read())
...
def rating_overall():
    ...
    comments_user1 = BeautifulSoup(open('dataset/comments.txt').read())
    comments_user1 = comments_user1.findAll('review_text')
    comments_user2 = BeautifulSoup(open('dataset/commentstwo.txt').read())
